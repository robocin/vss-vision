                    ...
                    [&](const tbb::blocked_range2d<int>& r) {
                      for (int i = r.rows().begin(); i != r.rows().end(); i++) {
                        ...
                        for (int j = r.cols().begin(); j != r.cols().end(); j++) {
                          ...
                          // std::cout << "Lut Segmentation [" << (int) label << "] = {" << (int)
                          // row[j].red << ", " << (int) row[j].green << ", " << (int) row[j].blue <<
                          // "}" << std::endl;
  ...
  for (int r = 0; r < 256; r++) {
    for (int g = 0; g < 256; g++) {
      for (int b = 0; b < 256; b++) {
        ...
        if (abs(r - g) < this->_grayTrashHoldLevel && abs(r - b) < this->_grayTrashHoldLevel &&
...
void LUTSegmentation::initFromFile(std::string path) {
  ...
  if (!fs.isOpened()) {
    spdlog::get("Vision")->error("LUTSegmentation::initFromFile: Diretorio não encontrado {}",
                                 path);
  }
