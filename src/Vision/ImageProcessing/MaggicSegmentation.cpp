...
void PREPARE_LUT_GPU() { initGPU(LUT_SIZE); }

// Make calculations to LUT
void LUT_GPU() { makeLUTGPU(LUT_SIZE / 3); }

void GET_LUT_GPU_DATA() { getLUTFromGPU(LUT_GPU_V, LUT_SIZE); }
void FINISH_LUT_GPU() { releaseGPU(); }

void APPLY_LUT_GPU(cv::Mat &mat) { applyLUTGPU(mat.data, mat.cols * mat.rows); }
...
MaggicSegmentation::MaggicSegmentation() {
  ...
  colorPalette.at<cv::Vec3b>(0, 7) =
      cv::Vec3b(252, 252, 119); // Azul Claro que bugava com verde- ou verde
  // colorPalette.at<cv::Vec3b>(0, 7) = cv::Vec3b(252, 197, 119); // Azul Claro
  // bom colorPalette.at<cv::Vec3b>(0, 2) = cv::Vec3b(255, 180, 90); // Azul
  // escuro teste

  // colorPalette.at<cv::Vec3b>(0, 7) = cv::Vec3b(252, 208, 0); // Azul Claro
  // melhor
  ...
  // colorPaletteHSV.at<cv::Vec3b>(0, 4)[0] = (colorPaletteHSV.at<cv::Vec3b>(0,
  // 4)[0])%256;
  ...
  for (int i = 0; i < 256; i++) {
    for (int j = 0; j < 256; j++) {
      cv::rectangle(colorPlane, cv::Rect(j << 1, i << 1, 2, 2),
                    cv::Scalar(j, 255, i), -1);
    }
  }
  for (int i = 0; i < 10; ++i) {
    ...
    // std::cout << e << std::endl;
    cv::rectangle(cores, cv::Rect(32, i * 32, 32, 32), cv::Scalar(e, 255, 255),
                  -1);
    cv::rectangle(cores, cv::Rect(0, i * 32, 32, 32), cv::Scalar(vec), -1);
    cv::line(compara, cv::Point(e << 1, 0), cv::Point(e << 1, 20),
             cv::Scalar(vec), 2);

    cv::circle(colorPlane, cv::Point(vec[0] << 1, vec[2] << 1), 5,
  ...
  this->_maggicSegmentationSessionFileName =
      "Config/Segmentation/MaggicSegmentation.txt";

  // std::cout << "THRESHOLD FROM MAGGIC SEGMENTATION ON CONSTRUCTING" <<
  // this->getFilterGrayThresholdValue() << std::endl;

  this->openLastSession();

  // std::cout << "THRESHOLD FROM MAGGIC SEGMENTATION ON CONSTRUCTING AFTER " <<
  // this->getFilterGrayThresholdValue() << std::endl;
}
...
String MaggicSegmentation::RGBHash2String(uint h) {
  ...
  if (preprocess) {
    ...
    for (uint i = 0; i < 8; ++i) { // black to light blue
      cv::Vec3b *v =
          reinterpret_cast<cv::Vec3b *>(ColorSpace::markerColors) + i;
...
void MaggicSegmentation::openLastSession() {
  ...
  if (fin.is_open()) {
    ...
    if (str.compare("Maggic") ==
        0) { // Test if the files has any valid content, then read it all
      fin >> this->_minimumGrayThreshold >> this->_maximumGrayThreshold;
      fin >> this->filterGrayThreshold;
      // std::cout << "THRESHOLD FROM MAGGIC SEGMENTATION FROM OPEN" <<
      // this->filterGrayThreshold << std::endl;
      this->hueList.clear();
      ...
      while (true) {
        ...
        if (fin.eof())
          break;
        this->hueList.push_back(
            std::make_pair(static_cast<float>(numa), static_cast<int>(numb)));
      }
    }
    this->setHUETable(true);
    this->generateLUTFromHUE();
    this->useLoadedValues = true;
    spdlog::get("Vision")->info(
        "MaggicSegmentation:: openLastSession: Loaded session from '{}'.",
        this->_maggicSegmentationSessionFileName);
  } else {
    this->useLoadedValues = false;
    spdlog::get("Vision")->warn(
        "MaggicSegmentation:: openLastSession: File access denied '{}'.",
        this->_maggicSegmentationSessionFileName);
  }
...
void MaggicSegmentation::saveSession() {
  ...
  if (fout.is_open()) {
    ...
    for (size_t i = 0; i < this->hueList.size(); i++) {
      fout << static_cast<int>(this->hueList[i].first) << " "
           << static_cast<int>(this->hueList[i].second) << "\n";
    }
    spdlog::get("Vision")->info(
        "MaggicSegmentation:: saveSession: Successfully saved in '{}'.",
        this->_maggicSegmentationSessionFileName);
  } else {
    spdlog::get("Vision")->error(
        "MaggicSegmentation:: saveSession: File access denied '{}'.",
        this->_maggicSegmentationSessionFileName);
  }
  fout.close();
  mut.unlock();
...
void MaggicSegmentation::calibrate(cv::Mat &frame) {
  ...
  // enableEstimateRobots = !(this->_debugSelection ==
  // MaggicVisionDebugSelection_DetailsFrame && this->paused);
  this->_manyTimes = 1;
  this->_entitiesCount = 7;
  // if (enableEstimateRobots) estimateRobots(image,this->_manyTimes,
  // this->_entitiesCount);
  if (this->m_updateFrame) {
    ...
    if (this->_debugSelection == MaggicVisionDebugSelection_SegmentationFrame) {
      ...
      cv::cvtColor(extremeSaturation, this->_extremeSaturation,
...
void MaggicSegmentation::setDebugSelection(
    MaggicVisionDebugSelection selection) {
  this->_debugSelection = selection;
}

void MaggicSegmentation::setManyTimes(int manyTimes) {
  if (manyTimes > 0)
    this->_manyTimes = manyTimes;
}

void MaggicSegmentation::setEntitiesCount(int entitiesCount) {
  if (entitiesCount > 0 && entitiesCount < 8)
    this->_entitiesCount = entitiesCount;
}

int MaggicSegmentation::getEntitiesCount() { return this->_entitiesCount; }
...
inline void MaggicSegmentation::filterGray(cv::Vec3b &color,
                                           cv::Vec3b &coloro) {
  float x = coloro[0];
  float y = coloro[1];
  float z = coloro[2];
  ...
  if (x >= lower_bound && x <= upper_bound && y >= lower_bound &&
...
void MaggicSegmentation::filterBinarizeColored(cv::Mat &d, cv::Mat &o) {
  ...
  for (int r = 0; r < o.rows; ++r) {
    for (int c = 0; c < o.cols; ++c) {
      ...
      if (coloro[0] == 0 && coloro[1] == 0 && coloro[2] == 0)
        res.at<cv::Vec3b>(r, c) = 0;
      else
...
void MaggicSegmentation::updateHistogramDescriptors() {
  ...
  if (colorDescriptors.size() == 0 ||
      static_cast<int>(colorDescriptors.back()) != last)
    colorDescriptors.push_back(last * div255);
}

void MaggicSegmentation::filterGrain(ColorDescriptor &dest,
                                     ColorDescriptor &orig) {
  if (orig.size() == 0)
    return;
  ...
  if (orig.size() == 1) {
    ...
    if (orig[1] - orig[0] > grainThreshold) {
      ...
      dest.push_back((orig[1] + orig[0]) * 0.5f);
    return;
  }
...
void MaggicSegmentation::_layeredAdd(cv::Mat &out, cv::Mat imgA, cv::Mat imgB) {
  CV_Assert(imgA.cols == imgB.cols && imgA.rows == imgB.rows &&
            imgA.channels() == imgB.channels() && imgA.channels() == 3);

  for (int i = 0; i < imgA.cols * imgA.rows * imgA.channels(); ++i) {
    int pixel = (i / imgA.channels()) * 3;
    int pixel1 = pixel + 1;
    int pixel2 = pixel + 2;
    if (imgB.data[pixel] == 0 && imgB.data[pixel1] == 0 &&
        imgB.data[pixel2] == 0) {
...
void MaggicSegmentation::setHUETable(bool fromFile) {
  ...
  if (!fromFile) { // if it's not from file run normally
    ...
    //    spdlog::get("Vision")->info("MaggicSegmentation:: Loading Default HUE
    //    list\n");
    ...
    for (int i = 1; i < 8; i++) {
      ...
      for (size_t j = 0; j < colors.size(); j++) {
        ...
        for (size_t k = 0; k < defaultHueList.size(); k++) {
          ...
          float y = hueChannel;
          float d = std::min(fabs(x - y),
                             std::min(fabs((256 + x) - y), fabs(x - y - 256)));
          if (d < bd) {
            bd = d;
  ...
  for (int i = 0; i < 256; i++) {
    ...
    for (size_t j = 0; j < hueList.size(); j++) {
      ...
      int d =
          std::min(abs(x - i), std::min(abs((256 + x) - i), abs(x - i - 256)));
  ...
  //  spdlog::get("Vision")->info("MaggicSegmentation:: Default HUE list
  //  loaded.\n");
}
...
void MaggicSegmentation::generateLUTFromHUE() {
  ...
  tbb::parallel_for(
      tbb::blocked_range<int>(0, LUTSIZE, 238),
  ...
  // spdlog::get("Vision")->info("MaggicSegmentation::Done generating LUT from
  // Hue.\n");
  this->isLUTReady = true;
}

uchar *MaggicSegmentation::getLUT() { return this->_LUT; }

bool MaggicSegmentation::estimateRobots(cv::Mat img, int manyTimes,
                                        int n_components_reference) {
  CV_Assert(img.empty() == false && manyTimes > 0 &&
            n_components_reference > 0);

  ++n_components_reference;
  static int original_filterGrayThreshold =
      rand() % _intervalGrayThreshold + _minimumGrayThreshold;
  ...
  if (this->_debugSelection > MaggicVisionDebugSelection_Undefined)
    this->_multipliedResults = img.clone();
  ...
  for (int times = 0; times < manyTimes && !found; ++times) {
    ...
    if (this->_learningThresholdValue)
      filterGrayThreshold = original_filterGrayThreshold + actualAdditive;
    ...
    // First pass to get the main connected components
    {
      ...
      // std::cout << "thresh t " << filterGrayThreshold << "actual additive "
      // << actualAdditive <<  std::endl;
      ...
      filterExtremeSaturation(extremeSaturation, extremeSaturation);
      cv::cvtColor(extremeSaturation, extremeSaturation,
                   cv::COLOR_HSV2BGR_FULL);
      cv::resize(
          extremeSaturation, extremeSaturation,
          cv::Size(extremeSaturation.cols << 1, extremeSaturation.rows << 1));
      // cv::imshow("ExtremeSaturation",extremeSaturation);
      if (this->_debugSelection > MaggicVisionDebugSelection_Undefined)
        this->_extremeSaturation = extremeSaturation.clone();
      ...
      n_components = cv::connectedComponentsWithStats(
          t, components, stats, centroids, 8, CV_32S, cv::CCL_DEFAULT);
      ...
      //#pragma omp parallel sections
      {
        ...
        //#pragma omp section
        {
          for (int i = 1; i < n_components; ++i) {
            componentsCentroids.push_back(
                cv::Point2i(static_cast<int>(centroids.at<double>(i, 0)),
            ...
            //	//cv::line(RB, cv::Point(0, 255 - xmed), cv::Point(256, 255 -
            // xmed), cv::Scalar(0, 255, 0));
            //	//cv::line(RB, cv::Point(0, 255 - colorDescriptor),
            // cv::Point(256, 255 - colorDescriptor), cv::Scalar(0, 0, 255));
            // for
            //(auto &e : colorDescriptors) { 		uchar hue = e * 255;
            // cv::line(RB, cv::Point(0, 255 - hue), cv::Point(256, 255 - hue),
            // cv::Scalar(0, 0, 255));
            //	}
            //}
    ...
    // imshow("components", t);
    // std::cout << "components amount and reference " << components_amount << "
    // " << n_components_reference << std::endl;
    ...
    // img.copyTo(d);
    // cv::resize(d, d, cv::Size(d.cols >> 1, d.rows >> 1),0,0);
    //        // Second pass to get the bigger connected components crossed with
    ...
    //          cv::Mat components, stats, centroids;
    //          //cv::resize(histo, histo, cv::Size(256, 1), 0, 0,
    //          INTER_NEAREST);

    //          n_components = cv::connectedComponentsWithStats(t, components,
    //          stats, centroids,8,CV_32S,cv::CCL_DEFAULT);
    //          //found = n_components == n_components_reference-1;
    //          std::cout << "n components and reference " << n_components << "
    //          " << n_components_reference << std::endl; if (n_components ==
    //          n_components_reference) {
    //            found = true;
    //          } else found = false;
    ...
    ////                  }
    ////                }
    ////                if (bestArea < 150 && bestArea > 8)
    /// componentsRectangles.push_back(brct); /              }
    //            }
    // for (int i = 1; i < n_components; ++i) {
    ...
    //	//cv::line(RB, cv::Point(0, 255 - xmed), cv::Point(256, 255 - xmed),
    // cv::Scalar(0, 255, 0));
    //	//cv::line(RB, cv::Point(0, 255 - colorDescriptor), cv::Point(256, 255 -
    // colorDescriptor), cv::Scalar(0, 0, 255)); 	for (auto &e :
    // colorDescriptors) { 		uchar hue = e * 255;
    // cv::line(RB, cv::Point(0, 255 - hue), cv::Point(256, 255 - hue),
    // cv::Scalar(0, 0, 255));
    //	}
    //}
    //          if (componentsRectangles.size() < (size_t)
    //          n_components_reference && componentsRectangles.size() > 9) {
    //            found = false;
    //            break;
    //          }
    {
      ...
      // std::cout << "componentsRectangles.size() " <<
      // componentsRectangles.size() <<  std::endl;
      //  Third pass
      for (size_t i = 0; i < componentsRectangles.size(); ++i) {
        memset(histo.data, 0,
               sizeof(float) * static_cast<uint>(histo.cols * histo.rows *
                                                 histo.channels()));
        ...
        // doHistogram(d, r, t);
        float maior =
            max_element_of<float>(reinterpret_cast<float *>(histo.data),
                                  static_cast<uint>(histo.cols * histo.rows));
        ...
        // std::cout << "robots descriptors[" << i << "] size " <<
        // robotsDescriptors[i].colors.size() <<  std::endl;
      ...
      for (int i = 0; i < 7; ++i)
        colorsize = std::max(
            colorsize, static_cast<int>(robotsDescriptors[i].colors.size()));
      colorsize++;
      cv::Mat cores = cv::Mat::zeros(cv::Size(colorsize * 32, 32 * 7), CV_8UC3);

      cv::cvtColor(t, t, cv::COLOR_BGR2HSV_FULL);
      for (size_t i = 0; i < 7; ++i) {
        cv::rectangle(cores, cv::Rect(0, static_cast<int>(i * 32), 32, 32),
                      cv::Scalar(i * 255 / 7, 255, 255), -1);
        cv::rectangle(t, componentsRectangles[i],
                      cv::Scalar(i * 255 / 7, 255, 255), -1);
        for (size_t ii = 0; ii < robotsDescriptors[i].colors.size(); ++ii) {
          uchar e = static_cast<uchar>(robotsDescriptors[i].colors[ii]);
          cv::rectangle(cores,
                        cv::Rect(static_cast<int>(ii * 32 + 32),
                                 static_cast<int>(i * 32), 32, 32),
                        cv::Scalar(e * 255, 255, 255), -1);
        }
      }
      cv::cvtColor(cores, cores, cv::COLOR_HSV2BGR_FULL);
      ...
      if (this->_debugSelection > MaggicVisionDebugSelection_Undefined)
        this->_multipliedResults = mt.clone();
      ...
      cv::cvtColor(colorPlane, colorPlane, cv::COLOR_HSV2BGR_FULL);
      cv::Scalar blackAndWhite[2] = {cv::Scalar(0, 0, 0),
                                     cv::Scalar(255, 255, 255)};
      // std::cout << "hueList.size() " << hueList.size() << std::endl;
      int begin = 0;
      int end = 1;
      for (size_t i = 0; i < hueList.size(); i++) {
        while (i + 1 < hueList.size() &&
               hueList[i + 1].second == hueList[i].second)
          i++;
        end = static_cast<int>(hueList[i].first) << 1;

        cv::line(colorPlane, cv::Point(begin, 500), cv::Point(end, 500),
                 blackAndWhite[i % 2], 4);
  ...
  if (this->_learningThresholdValue) {
    ...
    if (filterGrayThreshold < _minimumGrayThreshold ||
        filterGrayThreshold > _maximumGrayThreshold ||
        n_components != n_components_reference - 1)
      filterGrayThreshold =
          _maximumGrayThreshold; // rand()%_intervalGrayThreshold
                                 // +_minimumGrayThreshold;//filterGrayThreshold
                                 // = original_filterGrayThreshold;
  }
  // if (!found && this->_debugSelection ==
  // MaggicVisionDebugSelection_MultipliedResults) this->_segmentationFrame =
  // img.clone();
  //#endif
  return found; // return whether it found or not a threshold value that gets
                // only 7 components (if searching for all components, 3 players
                // x2 teams + 1 ball)
}

void MaggicSegmentation::setFilterGrayThresholdValue(
    int newFilterGrayThreshold) {
...
void MaggicSegmentation::getFilterGrayThresholdValues(int &minimum,
                                                      int &maximum) {
  minimum = _minimumGrayThreshold;
  maximum = _maximumGrayThreshold;
}

void MaggicSegmentation::setFilterGrayThresholdValues(int minimum,
                                                      int maximum) {
...
bool MaggicSegmentation::isLearning() { return this->_learningThresholdValue; }
...
cv::Mat MaggicSegmentation::run(cv::Mat &frame) {
  ...
  for (int i = 0; i < frame.rows; i++) {
    ...
    for (int j = 0; j < frame.cols; j++) {
      label =
          this->_LUT[(row[j].red << 16) + (row[j].green << 8) + row[j].blue];
...
void MaggicSegmentation::loadDefaultHue() {
  const static char *defaultHueListFileName =
      "Config/Segmentation/DefaultMaggicSegmentation.txt";
  ...
  if (error) {
    spdlog::get("Vision")->error(
        "MaggicSegmentation:: loadDefaultHue: File access denied {}",
        defaultHueListFileName);
  }
}
...
void MaggicSegmentation::setMousePosition(cv::Point2f mpos) {
  ...
  if (!this->_detailsFrame.empty()) {
    ...
    this->cursorPos =
        cv::Point2i(static_cast<int>(mpos.x * this->_detailsFrame.cols),
                    static_cast<int>(mpos.y * this->_detailsFrame.rows));
    this->mouseDrag = true;
  }
  mut.unlock();
}
...
void MaggicSegmentation::setMouseButtonPress(int btnId) {
  ...
  // this->m_updateFrame = this->m_updateDetails = true;
  if (!this->_detailsFrame.empty()) {
    if (cv::Rect(0, 0, this->_detailsFrame.cols, this->_detailsFrame.rows)
            .contains(this->cursorPos)) {
...
void MaggicSegmentation::setMouseButtonRelease(int btnId) {
  ...
  if (!this->_detailsFrame.empty()) {
    if (cv::Rect(0, 0, this->_detailsFrame.cols, this->_detailsFrame.rows)
      ...
      // std::cout << (btnId == 1 ? "Left" : "Right") << " Clicked" <<
      // std::endl;
    }
  }
  mut.unlock();
...
void MaggicSegmentation::doDetails() {
  ...
  static cv::Rect colorSelectionRecRef(colorFrame.x + colorFrame.width + 16,
                                       colorFrame.y + 2, 32, 32);
  ...
  if (this->_debugSelection == MaggicVisionDebugSelection_DetailsFrame) {
    ...
    if (this->_colorDetailsFrame.empty() || this->updateColors) {
      this->_colorDetailsFrame = cv::Mat::zeros(
          cv::Size(colorFrame.width, colorFrame.height), CV_8UC3);
      uchar h, s, v;
      ...
      cv::cvtColor(this->_colorDetailsFrame, this->_colorDetailsFrame,
                   cv::COLOR_HSV2BGR_FULL);
    ...
    if (lastCursorPos.size() > 0) {
      for (cv::Point2i &last : this->lastCursorPos) {
        cv::circle(this->_detailsFrame, last, selectionCircleRadius + 1,
                   cv::Scalar(0, 0, 0), -1);
      }
    ...
    if (this->colorDistribution) {
      ...
      cv::cvtColor(this->_imageBufferFiltered, this->_imageBufferHSV,
      ...
      for (int i = 0; i < this->_imageBufferHSV.rows; ++i) {
        ...
        for (int j = 0; j < this->_imageBufferHSV.cols; ++j) {
          if (!(rowRGB[j].blue == 0 && rowRGB[j].green == 0 &&
                rowRGB[j].red == 0)) {
            if (row[j].saturation > row[j].value) {
              this->_detailsFrame.at<cv::Vec3b>(
                  colorFrame.y + (h2 * row[j].value) / 255,
                  colorFrame.x + (colorFrame.width * row[j].hue) / 255) =
                  cv::Vec3b(255, 255, 255);
            } else {
              this->_detailsFrame.at<cv::Vec3b>(
                  colorFrame.y + h2 + ((255 - row[j].saturation) * h2) / 255,
                  colorFrame.x + (colorFrame.width * row[j].hue) / 255) =
                  cv::Vec3b(255, 255, 255);
            }
          }
    ...
    for (cv::Rect &r : this->filterRectangles) {
      cv::rectangle(this->_detailsFrame, r, cv::Scalar(0, 0, 0), -1,
                    cv::LINE_4);
      cv::rectangle(this->_detailsFrame, r, cv::Scalar(50, 50, 0), 1,
                    cv::LINE_4);
    }
    ...
    for (unsigned long i = 0; i < 7; i++) {
      ...
      if (colorSelection[i].contains(this->cursorPos)) {
        cv::rectangle(this->_detailsFrame, colorSelection[i],
                      cv::Scalar(0, 0, 0), 1);
    ...
    cv::rectangle(this->_detailsFrame,
                  cv::Rect(colorFrame.x - borderDistance + 1,
                           colorFrame.y - borderDistance + 1,
                           colorFrame.width + (borderDistance << 1) - 2,
                           colorFrame.height + (borderDistance << 1) - 2),
                  cv::Scalar(0, 0, 0), 4);
    cv::rectangle(this->_detailsFrame,
                  cv::Rect(colorFrame.x - borderDistance,
                           colorFrame.y - borderDistance,
                           colorFrame.width + (borderDistance << 1),
                           colorFrame.height + (borderDistance << 1)),
                  cv::Scalar(255, 255, 255), 1);
    int pivotId = -1;
    for (size_t i = 0; i < this->hueList.size(); i++) {
      std::pair<float, int> &hue = this->hueList[i];
      int theX = static_cast<int>(hue.first * colorFrame.width / 255 +
                                  colorFrame.x + 3);
      if (!this->enableFilter) {
        int theDist = abs(this->cursorPos.x - theX);
        if (theDist < 6 && colorFrame.contains(this->cursorPos)) {
          if (pivotId == -1 ||
              theDist < abs(this->cursorPos.x -
                            (this->hueList[static_cast<size_t>(pivotId)].first *
                                 colorFrame.width / 255 +
                             colorFrame.x))) {
            pivotId = static_cast<int>(i);
          }
        }
      }
      if (pivotForPaletteId - 1 == static_cast<int>(i))
        cv::line(this->_detailsFrame, cv::Point(theX, colorFrame.y + 1),
                 cv::Point(theX, colorFrame.y + colorFrame.height - 2),
                 cv::Scalar(255, 0, 255), 2);
      else if (this->dragpivotId != static_cast<int>(i))
        cv::line(this->_detailsFrame, cv::Point(theX, colorFrame.y),
                 cv::Point(theX, colorFrame.y + colorFrame.height - 1),
                 cv::Scalar(255, 255, 255), 1);
    }
    ...
    if (this->mouseDrag) {
      if (dragpivotId != -1) {
        ...
        int theX = std::max(
            colorFrame.x + 1,
            std::min(colorFrame.x + colorFrame.width - 2, this->cursorPos.x));
        cv::line(this->_detailsFrame, cv::Point(theX, colorFrame.y + 1),
                 cv::Point(theX, colorFrame.y + colorFrame.height - 2),
                 cv::Scalar(0, 255, 0), 2);
      }
      if (this->enableFilter && firstPress.x > -1) {
        static int mx = colorFrame.x, mX = colorFrame.x + colorFrame.width,
                   my = colorFrame.y, mY = colorFrame.y + colorFrame.height;
        ...
        cv::rectangle(this->_detailsFrame, tmpFilterRect, cv::Scalar(0, 0, 0),
                      -1, cv::LINE_4);
        cv::rectangle(this->_detailsFrame, tmpFilterRect,
    ...
    if (this->releasedLeft) {
      bool changed = false;
      if (this->dragpivotId != -1) {
        int theX = std::max(colorFrame.x + 1,
                            std::min(colorFrame.x + colorFrame.width - 2,
                                     this->cursorPos.x)) -
                   colorFrame.x - 3;
        float newHue = static_cast<float>(theX) * 255.0f /
                       static_cast<float>(colorFrame.width);
      ...
      if (this->enableFilter && firstPress.x > -1) {
        ...
        if (tmpFilterRect.width > 0 && tmpFilterRect.height > 0)
          this->filterRectangles.push_back(tmpFilterRect);
        else
          removeTopRectangle(this->filterRectangles, this->cursorPos);

        // regenerate mask
        if (this->_filterMask.empty())
          this->_filterMask = cv::Mat::zeros(256, 256, CV_8U);
        cv::rectangle(this->_filterMask, cv::Rect(0, 0, 255, 255),
      ...
      if (changed) {
        ...
        uchar *dst_LUT = reinterpret_cast<LUTSegmentation *>(
                             Vision::singleton().getSegmentationObject())
                             ->getLUT();
    ...
    if (this->releasedLeft) {
      if (pivotForPaletteId && colorSelectionId != -1) {
        this->hueList[static_cast<size_t>(pivotForPaletteId - 1)].second =
            colorSelectionId + 1;
        std::cout << "Selected color " << colorSelectionId + 1 << " for "
                  << pivotForPaletteId - 1 << std::endl;
        mut.unlock();
        saveSession();
        mut.lock();
    ...
    if (pivotForPaletteId) {
      int colorForPaletteId =
          this->hueList[static_cast<size_t>(pivotForPaletteId - 1)].second - 1;
    ...
    if (pivotId != -1 && this->dragpivotId == -1) {
      int theX = static_cast<int>(
          static_cast<float>(this->hueList[static_cast<size_t>(pivotId)].first *
                             colorFrame.width) /
              255.0f +
          static_cast<float>(colorFrame.x) + 3);
      cv::line(this->_detailsFrame, cv::Point(theX, colorFrame.y + 1),
               cv::Point(theX, colorFrame.y + colorFrame.height - 2),
               cv::Scalar(0, 255, 255), 2);
    }

    // draw vertical line for reference, only when mouse over color frame and
    // not over pivots
    ...
    cv::circle(this->_detailsFrame, this->cursorPos, selectionCircleRadius + 1,
               cv::Scalar(0, 0, 0), 1);
    cv::circle(this->_detailsFrame, this->cursorPos, selectionCircleRadius,
               cv::Scalar(255, 255, 255), 1);
    cv::circle(this->_detailsFrame, this->cursorPos, selectionCircleRadius - 1,
               cv::Scalar(0, 0, 0), 1);
...
void MaggicSegmentation::setNormalizedEnabled(bool enabled) {
  ...
  this->normalization_method =
      (enabled ? this->selected_normalization_method : NO_NORMALIZATION);
  this->updateColors = true;
  this->mut.unlock();
}
...
void MaggicSegmentation::setNormalizationMethod(
...
void MaggicSegmentation::getNormalizationMethod(
    MaggicSegmentation::NormalizationMethod &method) {
  this->mut.lock();
...
MaggicSegmentation::NormalizationMethod
MaggicSegmentation::getNormalizationMethod() {
...
void MaggicSegmentation::removeTopRectangle(Rectangles &rects, cv::Point &p) {
  ...
  if (it_to_remove != rects.end())
    rects.erase(it_to_remove);
}
...
void MaggicSegmentation::saveSelectedDebug() {
  ...
  if (this->_debugSelection == MaggicVisionDebugSelection_Thresholded) {
    // sprintf(fileName,"maggic_thresholded_%s%03d.png", normalizedOrNot,
    // filterGrayThreshold);
    ...
    // sprintf(fileName,"maggic_segmentation_%s%03d.png", normalizedOrNot,
    // filterGrayThreshold);
    ...
    std::cout << "Saved segmentation" << std::endl;
             ...
             MaggicVisionDebugSelection_ExtremeSaturation) {
    // sprintf(fileName,"maggic_extremeSaturation_%s%03d.png", normalizedOrNot,
    // filterGrayThreshold);
    ...
    // sprintf(fileName,"maggic_detailsFrame_%s%03d.png", normalizedOrNot,
    // filterGrayThreshold);
...
void MaggicSegmentation::lock() { mut.lock(); }

void MaggicSegmentation::unlock() { mut.unlock(); }
