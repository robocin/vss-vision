...
const MaggicSegmentation::NormalizationMethod MaggicSegmentation::default_normalization_method =
...
std::string MaggicSegmentation::RGBHash2String(uint h) {
  ...
  if (preprocess) {
    ...
    for (uint i = 0; i < 8; ++i) { // black to light blue
      ...
      colorNames.insert(
          std::make_pair(MaggicSegmentation::BGR2RGBHash(*v), ColorSpace::colorNames[i]));
    }
  }
  return colorNames[h];
}
...
void MaggicSegmentation::openLastSession() {
  ...
  if (fin.is_open()) {
    ...
    if (str.compare("Maggic") == 0) { // Test if the files has any valid content, then read it all
      ...
      // std::cout << "THRESHOLD FROM MAGGIC SEGMENTATION FROM OPEN" << this->filterGrayThreshold <<
      // std::endl;
      this->hueList.clear();
    ...
    spdlog::get("Vision")->info("MaggicSegmentation:: openLastSession: Loaded session from '{}'.",
                                this->_maggicSegmentationSessionFileName);
  } else {
    this->useLoadedValues = false;
    spdlog::get("Vision")->warn("MaggicSegmentation:: openLastSession: File access denied '{}'.",
                                this->_maggicSegmentationSessionFileName);
  }
...
void MaggicSegmentation::saveSession() {
  ...
  if (fout.is_open()) {
    ...
    spdlog::get("Vision")->info("MaggicSegmentation:: saveSession: Successfully saved in '{}'.",
                                this->_maggicSegmentationSessionFileName);
  } else {
    spdlog::get("Vision")->error("MaggicSegmentation:: saveSession: File access denied '{}'.",
                                 this->_maggicSegmentationSessionFileName);
  }
  fout.close();
  mut.unlock();
...
void MaggicSegmentation::calibrate(cv::Mat& frame) {
  ...
  // enableEstimateRobots = !(this->_debugSelection == MaggicVisionDebugSelection_DetailsFrame &&
  // this->paused);
  this->_manyTimes = 1;
  this->_entitiesCount = 7;
...
void MaggicSegmentation::updateHistogramDescriptors() {
  ...
  if (colorDescriptors.size() == 0 || static_cast<int>(colorDescriptors.back()) != last)
    colorDescriptors.push_back(last * div255);
}
...
bool MaggicSegmentation::estimateRobots(cv::Mat img, int manyTimes, int n_components_reference) {
  ...
  for (int times = 0; times < manyTimes && !found; ++times) {
    ...
    // First pass to get the main connected components
    {
      ...
      // std::cout << "thresh t " << filterGrayThreshold << "actual additive " << actualAdditive <<
      // std::endl;
      ...
      n_components = cv::connectedComponentsWithStats(t,
                                                      components,
                                                      stats,
                                                      centroids,
                                                      8,
      ...
      //#pragma omp parallel sections
      {
        ...
        //#pragma omp section
        {
          for (int i = 1; i < n_components; ++i) {
            ...
            //	//cv::line(RB, cv::Point(0, 255 - xmed), cv::Point(256, 255 - xmed), cv::Scalar(0,
            //255, 0));
            //	//cv::line(RB, cv::Point(0, 255 - colorDescriptor), cv::Point(256, 255 -
            //colorDescriptor), cv::Scalar(0, 0, 255)); 	for (auto &e : colorDescriptors) { 		uchar hue
            //= e * 255; 		cv::line(RB, cv::Point(0, 255 - hue), cv::Point(256, 255 - hue),
            //cv::Scalar(0, 0, 255));
            //	}
            //}
    ...
    // img.copyTo(d);
    // cv::resize(d, d, cv::Size(d.cols >> 1, d.rows >> 1),0,0);
    //        // Second pass to get the bigger connected components crossed with main connected
    //        components
    //        {
    //          //cv::GaussianBlur(d, d, cv::Size(7, 7), 3, 3);
    ...
    //          n_components = cv::connectedComponentsWithStats(t, components, stats,
    //          centroids,8,CV_32S,cv::CCL_DEFAULT);
    //          //found = n_components == n_components_reference-1;
    //          std::cout << "n components and reference " << n_components << " " <<
    //          n_components_reference << std::endl; if (n_components == n_components_reference) {
    //            found = true;
    //          } else found = false;
    ...
    //	//cv::line(RB, cv::Point(0, 255 - colorDescriptor), cv::Point(256, 255 - colorDescriptor),
    //cv::Scalar(0, 0, 255)); 	for (auto &e : colorDescriptors) { 		uchar hue = e * 255; 		cv::line(RB,
    //cv::Point(0, 255 - hue), cv::Point(256, 255 - hue), cv::Scalar(0, 0, 255));
    //	}
    //}
    //          if (componentsRectangles.size() < (size_t) n_components_reference &&
    //          componentsRectangles.size() > 9) {
    //            found = false;
    //            break;
    //          }
    {
      ...
      for (size_t i = 0; i < componentsRectangles.size(); ++i) {
        memset(histo.data,
               0,
               sizeof(float) * static_cast<uint>(histo.cols * histo.rows * histo.channels()));
        ...
        // doHistogram(d, r, t);
        float maior = max_element_of<float>(reinterpret_cast<float*>(histo.data),
      ...
      int colorsize = 0;
      for (int i = 0; i < 7; ++i)
        colorsize = max(colorsize, static_cast<int>(robotsDescriptors[i].colors.size()));
      colorsize++;
      ...
      for (size_t i = 0; i < 7; ++i) {
        cv::rectangle(cores,
                      cv::Rect(0, static_cast<int>(i * 32), 32, 32),
                      cv::Scalar(i * 255 / 7, 255, 255),
                      -1);
        ...
        for (size_t ii = 0; ii < robotsDescriptors[i].colors.size(); ++ii) {
          ...
          cv::rectangle(cores,
                        cv::Rect(static_cast<int>(ii * 32 + 32), static_cast<int>(i * 32), 32, 32),
                        cv::Scalar(e * 255, 255, 255),
                        -1);
        }
      }
      cv::cvtColor(cores, cores, cv::COLOR_HSV2BGR_FULL);
...

      {
        for (int i = 0; i < 8; i++) {
          for (size_t j = 0; j < robotsDescriptors[i].colors.size(); j++) {
            ...
            vec[1] = 0;
            vec[2] = 255;
            cv::circle(colorPlane,
                       cv::Point(vec[0] << 1, vec[2] << 1),
                       5,
                       cv::Scalar(255, 0, 255),
                       -1);
          }
      ...
      for (size_t i = 0; i < hueList.size(); i++) {
        while (i + 1 < hueList.size() && hueList[i + 1].second == hueList[i].second)
          i++;
  ...
  if (this->_learningThresholdValue) {
    ...
    if (filterGrayThreshold < _minimumGrayThreshold ||
        filterGrayThreshold > _maximumGrayThreshold || n_components != n_components_reference - 1)
      filterGrayThreshold = _maximumGrayThreshold; // rand()%_intervalGrayThreshold
                                                   // +_minimumGrayThreshold;//filterGrayThreshold =
                                                   // original_filterGrayThreshold;
  }
  // if (!found && this->_debugSelection == MaggicVisionDebugSelection_MultipliedResults)
  // this->_segmentationFrame = img.clone();
  //#endif
  return found; // return whether it found or not a threshold value that gets only 7 components (if
                // searching for all components, 3 players x2 teams + 1 ball)
}
...
void MaggicSegmentation::getDebugFrame(cv::Mat& frame) {
  mut.lock();
  switch (this->_debugSelection) {
    case MaggicVisionDebugSelection_Thresholded: this->_firstThreshold.copyTo(frame); break;
    case MaggicVisionDebugSelection_ExtremeSaturation:
    ...
    case MaggicVisionDebugSelection_SegmentationFrame:
      ...
      break;
    case MaggicVisionDebugSelection_DetailsFrame: this->_detailsFrame.copyTo(frame); break;
    default: break;
  }
  mut.unlock();
}
...
void MaggicSegmentation::loadDefaultHue() {
  ...
  if (error) {
    spdlog::get("Vision")->error("MaggicSegmentation:: loadDefaultHue: File access denied {}",
...
void MaggicSegmentation::setMousePosition(cv::Point2f mpos) {
  ...
  if (!this->_detailsFrame.empty()) {
    ...
    this->cursorPos = cv::Point2i(static_cast<int>(mpos.x * this->_detailsFrame.cols),
...
void MaggicSegmentation::doDetails() {
  ...
  if (this->_debugSelection == MaggicVisionDebugSelection_DetailsFrame) {
    ...
    if (this->_colorDetailsFrame.empty() || this->updateColors) {
      this->_colorDetailsFrame =
          cv::Mat::zeros(cv::Size(colorFrame.width, colorFrame.height), CV_8UC3);
      uchar h, s, v;
    ...
    if (this->colorDistribution) {
      ...
      for (int i = 0; i < this->_imageBufferHSV.rows; ++i) {
        ...
        for (int j = 0; j < this->_imageBufferHSV.cols; ++j) {
          if (!(rowRGB[j].blue == 0 && rowRGB[j].green == 0 && rowRGB[j].red == 0)) {
            if (row[j].saturation > row[j].value) {
              this->_detailsFrame.at<cv::Vec3b>(colorFrame.y + (h2 * row[j].value) / 255,
                                                colorFrame.x + (colorFrame.width * row[j].hue) /
                                                                   255) = cv::Vec3b(255, 255, 255);
            } else {
              this->_detailsFrame.at<cv::Vec3b>(
                  colorFrame.y + h2 + ((255 - row[j].saturation) * h2) / 255,
                  colorFrame.x + (colorFrame.width * row[j].hue) / 255) = cv::Vec3b(255, 255, 255);
            }
          }
        }
      }
    ...
    for (size_t i = 0; i < this->hueList.size(); i++) {
      ...
      if (!this->enableFilter) {
        ...
        if (theDist < 6 && colorFrame.contains(this->cursorPos)) {
          if (pivotId == -1 ||
              theDist < abs(this->cursorPos.x - (this->hueList[static_cast<size_t>(pivotId)].first *
    ...
    if (this->mouseDrag) {
      if (dragpivotId != -1) {
        ...
        int theX =
            max(colorFrame.x + 1, min(colorFrame.x + colorFrame.width - 2, this->cursorPos.x));
        cv::line(this->_detailsFrame,
                 cv::Point(theX, colorFrame.y + 1),
                 cv::Point(theX, colorFrame.y + colorFrame.height - 2),
                 cv::Scalar(0, 255, 0),
      ...
      if (this->enableFilter && firstPress.x > -1) {
        static int mx = colorFrame.x, mX = colorFrame.x + colorFrame.width, my = colorFrame.y,
    ...
    if (this->releasedLeft) {
      ...
      if (changed) {
        this->generateLUTFromHUE();
        uchar* dst_LUT =
            reinterpret_cast<LUTSegmentation*>(Vision::singleton().getSegmentationObject())
                ->getLUT();
    ...
    if (this->releasedLeft) {
      if (pivotForPaletteId && colorSelectionId != -1) {
        ...
        std::cout << "Selected color " << colorSelectionId + 1 << " for " << pivotForPaletteId - 1
                  << std::endl;
        mut.unlock();
        saveSession();
        mut.lock();
    ...
    if (pivotId != -1 && this->dragpivotId == -1) {
      int theX = static_cast<int>(
          static_cast<float>(this->hueList[static_cast<size_t>(pivotId)].first * colorFrame.width) /
              255.0f +
...
void MaggicSegmentation::setNormalizedEnabled(bool enabled) {
  ...
  this->normalization_method = (enabled ? this->selected_normalization_method : NO_NORMALIZATION);
  this->updateColors = true;
  this->mut.unlock();
}
...
void MaggicSegmentation::saveSelectedDebug() {
  ...
  normalizedOrNot[0] = 0;
  if (this->normalized_color)
    sprintf(normalizedOrNot, "normalized_");
