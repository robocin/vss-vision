    ...
    for(int g = 1 ; g < this->_runLengthEncoding.size() ; g++){
        ...
        for(int h  = 0 ; h < this->_runLengthEncoding[g].size() ; h++){
            ...
            int end = this->_runLengthEncoding[g][h].start + this->_runLengthEncoding[g][h].width;
                ...
                while(g-1 >= 0 && lastLine < this->_runLengthEncoding[g-1].size() && this->_runLengthEncoding[g-1][lastLine].start <= end){ //não sei por que que ta dando o banzo ainda

                    if(this->_runLengthEncoding[g-1][lastLine].start + this->_runLengthEncoding[g-1][lastLine].width >= this->_runLengthEncoding[g][h].start && this->_runLengthEncoding[g-1][lastLine].color == cor){
                            ...
                            if(this->_runLengthEncoding[g-1][lastLine].parent == -1){
                                        ...
                                        this->_runLengthEncoding[g][h].areaBlob += this->_runLengthEncoding[g-1][lastLine].areaBlob;
                                        this->_runLengthEncoding[g][h].mediax += this->_runLengthEncoding[g-1][lastLine].mediax;
                                        this->_runLengthEncoding[g][h].mediay += this->_runLengthEncoding[g-1][lastLine].mediay;
    ...
    for(int g = 0 ; g < this->_runLengthEncoding.size() ; g++){
        ...
        for(int h  = 0 ; h < this->_runLengthEncoding[g].size() ; h++){
            if(this->_runLengthEncoding[g][h].parent == -1){
                 if((this->_runLengthEncoding[g][h].areaBlob >= this->_minSize &&  this->_runLengthEncoding[g][h].areaBlob <= this->_maxSize) ||
                        (this->_runLengthEncoding[g][h].color == OrangeCOL && this->_runLengthEncoding[g][h].areaBlob >= this->_minSizeBall && this->_runLengthEncoding[g][h].areaBlob <=this->_maxSizeBall)){
                     cor = this->_runLengthEncoding[g][h].color;
                     if(_numberOfCluster[cor] < NUMBEROFCOLOR){
                    blob[cor][_numberOfCluster[cor]].position.x = this->_runLengthEncoding[g][h].mediay / this->_runLengthEncoding[g][h].areaBlob;
                    blob[cor][_numberOfCluster[cor]].position.y = this->_runLengthEncoding[g][h].mediax / this->_runLengthEncoding[g][h].areaBlob;
                    ...
                    cv::circle(debugFrame,blob[cor][_numberOfCluster[cor]-1].position,5,cv::Scalar(255,255,255),2);
                    }
                }
            }
