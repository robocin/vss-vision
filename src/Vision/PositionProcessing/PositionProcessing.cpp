...
void PositionProcessing::findTeam(Players& players,
                                  cv::Mat& debugFrame,
  ...
  for (Region& region : teamRegions) {
    if (region.distance < blobMaxDist()) {
      ...
      auto& playerPosVel =
          _kalmanFilterRobots[0][robot.id() % 100].update(newPosition.x, newPosition.y);

      Geometry::PT filtPoint(playerPosVel(0, 0), playerPosVel(1, 0));
      Geometry::PT PlayVel(playerPosVel(2, 0), playerPosVel(3, 0));

      auto& playerRotVel =
          _dirFilteRobots[0][robot.id() % 100].update(std::cos(newAngle), std::sin(newAngle));
      ...
      cv::circle(debugFrame,
                 Utils::convertPositionCmToPixel(Point(filtPoint.x, filtPoint.y)),
                 15,
                 _colorCar[colorIndex + 1],
                 1,
                 cv::LINE_AA);
    }
  }
...
void PositionProcessing::findEnemys(Entities& players,
                                    cv::Mat& debugFrame,
                                    std::vector<Region>& enemyRegions) {
...
void PositionProcessing::findBall(Entity& ball, cv::Mat& debugFrame) {
  ...
  if (!blobBall.valid) {
    int fps = 30;
    auto& ballPosVel =
        _kalmanFilterBall[0][0].update(this->_ballLastPosition.x, this->_ballLastPosition.y);
    ...
    cv::circle(debugFrame,
               Utils::convertPositionCmToPixel(
                   cv::Point(static_cast<int>(filtPoint.x), static_cast<int>(filtPoint.y))),
               9,
               _colorCar[OrangeCOL],
               2,
               cv::LINE_AA);
  ...
  cv::circle(debugFrame,
             Utils::convertPositionCmToPixel(
                 cv::Point(static_cast<int>(newPosition.x), static_cast<int>(newPosition.y))),
             9,
             _colorCar[OrangeCOL],
             2,
             cv::LINE_AA);
  ...
  // Utils::convertPositionCmToPixel(cv::Point(filtPoint.x,filtPoint.y)),Utils::convertPositionCmToPixel(cv::Point(filtPoint.x+ballVel.x,filtPoint.y+ballVel.y)),_colorCar[OrangeCOL],
  // 2);
  this->_ballLastPosition =
      cv::Point(static_cast<int>(newPosition.x), static_cast<int>(newPosition.y));
}

std::pair<PositionProcessing::Blob, PositionProcessing::NearestBlobInfo>
PositionProcessing::getNearestPrimary(Blob current) {
  ...
  for (int teamColor = Color::BLUE; teamColor <= Color::YELLOW; teamColor++) {
    for (int i = 0; i < CLUSTERSPERCOLOR; i++) {
      if (blob[teamColor][i].valid) {
        distance =
            static_cast<int>(Utils::sumOfSquares(current.position, blob[teamColor][i].position));
