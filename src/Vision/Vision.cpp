...
void Vision::update(cv::Mat& frame, QTime timeStamp) {
  ...
  if (this->_deepLogRecord) {
    ...
    if (this->_firstFrameDeepLog) {
      ...
      std::stringstream ss;
      uint ini = actualTime;
      ss << this->_deepLogFilePath << "_" << timeStamp.hour() << "." << timeStamp.minute() << "."
         << timeStamp.second() << ".csv";
      ...
      if (this->_deepLogFile) {
        ...
        fprintf(this->_deepLogFile,
                "%s;%d;%d;%d;%d\n",
                this->_deepLogFileName.c_str(),
                this->_deepLogInitialTime,
                entities[ROBOT1].id(),
                entities[ROBOT2].id(),
                entities[ROBOT3].id());
        fprintf(
            this->_deepLogFile,
            "R1X;R1Y;R1T;R2X;R2Y;R2T;R3X;R3Y;R3T;A1X;A1Y;A1T;A2X;A2Y;A2T;A3X;A3Y;A3T;BX;BY;TIMESTAMP_FRAME\n");
      }
    }
    ...
    if (this->_deepLogFile) {
      ...
      fprintf(this->_deepLogFile,
              "%lf;%lf;%lf;",
              entities[ROBOT1].position().x,
              entities[ROBOT1].position().y,
              entities[ROBOT1].angle());
      fprintf(this->_deepLogFile,
              "%lf;%lf;%lf;",
              entities[ROBOT2].position().x,
              entities[ROBOT2].position().y,
              entities[ROBOT2].angle());
      fprintf(this->_deepLogFile,
              "%lf;%lf;%lf;",
              entities[ROBOT3].position().x,
              entities[ROBOT3].position().y,
              entities[ROBOT3].angle());

      fprintf(this->_deepLogFile,
              "%lf;%lf;%lf;",
              entities[ADV1].position().x,
              entities[ADV1].position().y,
              entities[ADV1].angle());
      fprintf(this->_deepLogFile,
              "%lf;%lf;%lf;",
              entities[ADV2].position().x,
              entities[ADV2].position().y,
              entities[ADV2].angle());
      fprintf(this->_deepLogFile,
              "%lf;%lf;%lf;",
              entities[ADV3].position().x,
              entities[ADV3].position().y,
              entities[ADV3].angle());

      fprintf(this->_deepLogFile,
              "%lf;%lf;%lf",
              entities[BALL].position().x,
              entities[BALL].position().y,
              entities[BALL].angle());
      fprintf(this->_deepLogFile, "%u\n", actualTime - this->_deepLogInitialTime);
    }
  }
  this->_visionFrameTimer.stop();
  this->_visionRunTime = static_cast<double>(this->_visionFrameTimer.getMilliseconds() * 0.2 +
                                             this->_visionRunTime * 0.8);
  this->_visionStatusLocker.unlock();
}
...
void Vision::setDeepLogFileName(std::string fileName) {
  ...
  this->_deepLogFileName =
      fileName.substr(std::max(static_cast<uint>(0), static_cast<uint>(fileName.rfind('/') + 1)));
  this->_visionStatusLocker.unlock();
}
