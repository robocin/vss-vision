...
MaggicSegmentationDialog::MaggicSegmentationDialog(QWidget *parent)
    : QDialog(parent), ui(new Ui::MaggicSegmentationDialog) {
  ...
  this->maggicSegmentation->setFilterGrayThresholdValues(
      FILTER_GRAY_THRESHOLD_DEFAULT_MINIMUM,
      FILTER_GRAY_THRESHOLD_DEFAULT_MAXIMUM);
  this->maggicSegmentation->setDebugSelection(
      MaggicVisionDebugSelection_ExtremeSaturation);
  this->maggicSegmentation->setLearningThresholdFrames(
      static_cast<uint>(ui->learnSpinBox->value()));
  ...
  this->ui->threshGrayHorizontalSlider->setValue(
      this->maggicSegmentation->getFilterGrayThresholdValue());
  // std::cout << "THRESHOLD FROM MAGGIC SEGMENTATION " <<
  // this->maggicSegmentation->getFilterGrayThresholdValue() << std::endl;
  ...
  this->_groundTruthFileList =
      getFileList(_groundTruthFolderName, &(this->_groundTruthDir));
  ...
  this->ui->NormalizationComboBox->setCurrentIndex(
      static_cast<int>(this->maggicSegmentation->getNormalizationMethod()));
}
...
MaggicSegmentationDialog::~MaggicSegmentationDialog() {
  ...
  // disconnect(this->_updateFrameTimer,SIGNAL(timeout()),this,
  // SLOT(updateFrame())); this->close();
...
void MaggicSegmentationDialog::updateFrame() {
  if (this->isHidden())
    return;
  if (this->_updateFrameTimer->isActive() == false)
    return;
  ...
  if (!this->_actualFrame.empty()) {
    cv::resize(this->_actualFrame, this->_actualFrame, cv::Size(640, 480), 0,
               0);
...
void MaggicSegmentationDialog::setFrameOnScreen() {
  if (!this->_segmentedFrame.empty()) {
    ...
    QImage qimg2(reinterpret_cast<uchar *>(tempFrame.data), tempFrame.cols,
                 tempFrame.rows, static_cast<int>(tempFrame.step),
...
#define entitiesButtonClicked(elbo)                                            \
  void MaggicSegmentationDialog::on_entitiesButton_##elbo##_clicked() {        \
    this->maggicSegmentation->setEntitiesCount(elbo);                          \
    this->clearEntitiesCheck();                                                \
    this->entities[elbo]->setChecked(true);                                    \
  }
entitiesButtonClicked(1) entitiesButtonClicked(2) entitiesButtonClicked(3)
    entitiesButtonClicked(4) entitiesButtonClicked(5) entitiesButtonClicked(6)
...
void MaggicSegmentationDialog::on_tabWidget_currentChanged(int index) {
  this->maggicSegmentation->setDebugSelection(
      static_cast<MaggicVisionDebugSelection>(
          maggicVisionDebugSelectionVector[index + 1]));
}
...
void MaggicSegmentationDialog::on_resetButton_clicked() {
  ...
  this->maggicSegmentation->setFilterGrayThresholdValues(
      ...
      FILTER_GRAY_THRESHOLD_DEFAULT_MAXIMUM);
  this->maggicSegmentation->setDebugSelection(
      maggicVisionDebugSelectionVector[ui->tabWidget->currentIndex()]);
  this->maggicSegmentation->setLearningThresholdFrames(
      static_cast<uint>(ui->learnSpinBox->value()));
...
void MaggicSegmentationDialog::processImage(cv::Mat &ground_truth,
                                            cv::Mat &segmented,
                                            Statistics *gstats = nullptr,
                                            Statistics *sstats = nullptr) {
  if (gstats == nullptr || sstats == nullptr) {
    spdlog::get("MaggicVision")
        ->error(" gstats or sstats is nullptr (function : "
                "MaggicSemgentationDialog::processImage)");
    return;
  }
  ...
  tbb::parallel_for(
      tbb::blocked_range2d<int>(0, mask.rows, 238, 0, mask.cols, 161),
...
void MaggicSegmentationDialog::processBatch() {
  ...
  for (size_t i = 0; i < this->_inputFileList.size(); ++i) {
    ...
    for (int n = 0;
         n <
         MaggicSegmentation::NormalizationMethod::NORMALIZATION_METHODS_LENGTH;
         ++n) {
...
void MaggicSegmentationDialog::saveStats(Statistics &gstats,
                                         Statistics &sstats) {
  ...
  for (uint i = 0; i < 8; ++i) {         // black to light blue
    cv::Vec3b *v =
        reinterpret_cast<cv::Vec3b *>(ColorSpace::markerColors) + idx[i];
  ...
  if (plot) {
    ...
    int method_number = (this->maggicSegmentation->getNormalizedEnabled()
    ...
    if (fout.is_open()) {
      ...
      for (int i = 0; i < 8; ++i) {
        float pct = sstats[ordered_color[i]] /
                    static_cast<float>(gstats[ordered_color[i]]);
        fout << std::fixed << std::setprecision(1) << 100.0 * pct;
        if (i != 7)
          fout << "\t";
      }
      fout << "\t" << 100.0 * totalSG / static_cast<float>(totalGT)
           << std::endl;

      fout.close();
    } else {
      spdlog::get("MaggicVision")
          ->error("Could not save latex stats. (function : "
                  "MaggicSemgentationDialog::saveStats)");
    }
  }
  ...
  //                +"/"+this->_inputFileName.substr(0,fileNameUsableLen)
  //                +"_"+
  //                std::to_string(this->maggicSegmentation->getFilterGrayThresholdValue())
  //                +".tex";
  ...
  //                totalGT += gstats.find(ordered_color[i])->second;
  //                fout <<
  //                MaggicSegmentation::RGBHash2String(ordered_color[i]); if (i
  //                !=7) fout << " & ";
  //            }
  //            fout << "\\" << std::endl;
  ...
  //            for (int i =0; i<8; ++i) {
  //                float pct =
  //                sstats[ordered_color[i]]/static_cast<float>(gstats[ordered_color[i]]);
  //                fout << std::fixed << std::setprecision(1) << 100.0*pct;
  //                if (i !=7) fout << " & ";
  //            }
  //            fout << "\\" << std::endl;
  ...
  //            fout.close();
  //        } else {
  //            spdlog::get("MaggicVision")->error("Could not save latex stats.
  //            (function : MaggicSemgentationDialog::saveStats)");
  //        }
  //    }
  //    {
  //        size_t fileNameUsableLen = this->_inputFileName.size()-4;
  ...
  //            for (auto &e : gstats) {
  //                fout << MaggicSegmentation::RGBHash2String(e.first) << "\t"
  //                << e.second << std::endl; totalGT += e.second;
  //            }
  //            fout << "Segmented Stats:" << std::endl;
  //            for (auto &e : sstats) {
  //                fout << MaggicSegmentation::RGBHash2String(e.first) << "\t"
  //                << e.second << std::endl; totalSG += e.second;
  //            }
  //            fout << "Correct:" << std::endl;
  //            for (auto &e : sstats) {
  //                String colorName =
  //                MaggicSegmentation::RGBHash2String(e.first); float pct =
  //                e.second/static_cast<float>(gstats[e.first]); fout <<
  //                colorName << "\t" << std::fixed << std::setprecision(3) <<
  //                pct << std::endl;
  //            }
  //            fout << "Wrong:" << std::endl;
  //            for (auto &e : sstats) {
  ...
  //            fout << "SGpct\t" << std::fixed << std::setprecision(3) <<
  //            totalSG/static_cast<float>(totalGT) << std::endl; fout <<
  //            "SGpct\t" << std::fixed << std::setprecision(3) <<
  //            (totalGT-totalSG)/static_cast<float>(totalGT) << std::endl;

  //            fout.close();
  //        } else {
  //            spdlog::get("MaggicVision")->error("Could not save stats.
  //            (function : MaggicSemgentationDialog::saveStats)");
  //        }
  //    }
}
...
void MaggicSegmentationDialog::processAndSave() {
  ...
  //    this->processImage(this->_groundTruthFrame, this->_segmentedFrame,
  //    &gstats, &sstats);
  //    cv::imwrite(this->_outputDir->absolutePath().toStdString()
  //                +"/"
...
void MaggicSegmentationDialog::useNextImage() { // Circular function
  ...
  String input_impath = this->_inputDir->absolutePath().toStdString() + "/" +
                        this->_inputFileName;
  String ground_truth_impath =
      this->_groundTruthDir->absolutePath().toStdString() + "/" +
...
bool MaggicSegmentationDialog::eventFilter(QObject *f_object, QEvent *f_event) {
  ...
  if (f_event->type() == QEvent::MouseButtonPress) {
    ...
    if (this->maggicSegmentation) {
      this->maggicSegmentation->setMouseButtonPress(
  ...
  if (f_event->type() == QEvent::MouseButtonRelease) {
    QMouseEvent *mouseEvent = static_cast<QMouseEvent *>(f_event);
    if (this->maggicSegmentation) {
      this->maggicSegmentation->setMouseButtonRelease(
          static_cast<int>(mouseEvent->button()));
  ...
  if (f_event->type() == QEvent::MouseMove) {
    ...
    if (f_object == this) {
      ...
      QMouseEvent *mouseEvent = static_cast<QMouseEvent *>(f_event);
      // std::cout << "(" << qrect.x() << " " << qrect.y() << " " <<
      // qrect.width() << " " << qrect.height() << " )" << std::endl;
      cursorInsideVisualization = qrect.contains(
          static_cast<int>(mouseEvent->x()), static_cast<int>(mouseEvent->y()));
      // std::cout << "cursor " << (cursorInsideVisualization ? "inside" :
      // "outside") << std::endl;
      ...
      if (cursorInsideVisualization) {
        ...
        if (this->ui->tabWidget->currentIndex() == 4)
          this->ui->visualizationLabel->setCursor(Qt::BlankCursor);
        else
          this->ui->visualizationLabel->setCursor(Qt::ArrowCursor);
...
void MaggicSegmentationDialog::on_NormalizationComboBox_currentIndexChanged(
    int index) {
  this->maggicSegmentation->setNormalizationMethod(
      static_cast<MaggicSegmentation::NormalizationMethod>(index));
