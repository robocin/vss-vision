...
MaggicSegmentationDialog::MaggicSegmentationDialog(QWidget* parent) :
  ...
  this->maggicSegmentation->setFilterGrayThresholdValues(FILTER_GRAY_THRESHOLD_DEFAULT_MINIMUM,
  ...
  uchar* dst_LUT =
      reinterpret_cast<LUTSegmentation*>(Vision::singleton().getSegmentationObject())->getLUT();
...
void MaggicSegmentationDialog::setFrameOnScreen() {
  if (!this->_segmentedFrame.empty()) {
    ...
    spdlog::get("General")->warn(
        "You are setting a empty frame to QT Interface (function : SegmentationConfigDialog::setFrameOnScreen)");
  }
}
...
entitiesButtonClicked(1) entitiesButtonClicked(2) entitiesButtonClicked(3) entitiesButtonClicked(4)
    entitiesButtonClicked(5) entitiesButtonClicked(6) entitiesButtonClicked(7)
...
void MaggicSegmentationDialog::on_tabWidget_currentChanged(int index) {
  this->maggicSegmentation->setDebugSelection(
      static_cast<MaggicVisionDebugSelection>(maggicVisionDebugSelectionVector[index + 1]));
}
...
void MaggicSegmentationDialog::processImage(cv::Mat& ground_truth,
  ...
  if (gstats == nullptr || sstats == nullptr) {
    spdlog::get("MaggicVision")
        ->error(" gstats or sstats is nullptr (function : MaggicSemgentationDialog::processImage)");
    return;
  }
...
void MaggicSegmentationDialog::processBatch() {
  ...
  for (size_t i = 0; i < this->_inputFileList.size(); ++i) {
    this->useNextImage();
    for (int n = 0; n < MaggicSegmentation::NormalizationMethod::NORMALIZATION_METHODS_LENGTH;
         ++n) {
...
void MaggicSegmentationDialog::saveStats(Statistics& gstats, Statistics& sstats) {
  ...
  if (plot) {
    ...
    int method_number =
        (this->maggicSegmentation->getNormalizedEnabled() ? static_cast<int>(method) : 0);
    ...
    if (fout.is_open()) {
      ...
      spdlog::get("MaggicVision")
          ->error("Could not save latex stats. (function : MaggicSemgentationDialog::saveStats)");
    }
  }
  ...
  //        std::string filepath = this->_outputDir->absolutePath().toStdString()
  //                +"/"+this->_inputFileName.substr(0,fileNameUsableLen)
  //                +"_"+ std::to_string(this->maggicSegmentation->getFilterGrayThresholdValue())
  //                +".tex";
  ...
  //            for (int i =0; i<8; ++i) {
  //                float pct =
  //                sstats[ordered_color[i]]/static_cast<float>(gstats[ordered_color[i]]); fout <<
  //                std::fixed << std::setprecision(1) << 100.0*pct; if (i !=7) fout << " & ";
  //            }
  //            fout << "\\" << std::endl;

  //            fout.close();
  //        } else {
  //            spdlog::get("MaggicVision")->error("Could not save latex stats. (function :
  ...
  //            for (auto &e : gstats) {
  //                fout << MaggicSegmentation::RGBHash2String(e.first) << "\t" << e.second <<
  //                std::endl; totalGT += e.second;
  //            }
  //            fout << "Segmented Stats:" << std::endl;
  //            for (auto &e : sstats) {
  //                fout << MaggicSegmentation::RGBHash2String(e.first) << "\t" << e.second <<
  //                std::endl; totalSG += e.second;
  //            }
  ...
  //                fout << colorName << "\t" << std::fixed << std::setprecision(3) << pct <<
  //                std::endl;
  //            }
  //            fout << "Wrong:" << std::endl;
  //            for (auto &e : sstats) {
  ...
  //            totalSG/static_cast<float>(totalGT) << std::endl; fout << "SGpct\t" << std::fixed <<
  //            std::setprecision(3) << (totalGT-totalSG)/static_cast<float>(totalGT) << std::endl;

  //            fout.close();
  //        } else {
  //            spdlog::get("MaggicVision")->error("Could not save stats. (function :
  //            MaggicSemgentationDialog::saveStats)");
  //        }
  //    }
}
...
void MaggicSegmentationDialog::useNextImage() { // Circular function
  ...
  this->_selectedFileIndex =
      (this->_selectedFileIndex + 1) % static_cast<int>(this->_inputFileList.size());
  this->_inputFileName = this->_inputFileList[this->_selectedFileIndex];
  std::string input_impath =
      this->_inputDir->absolutePath().toStdString() + "/" + this->_inputFileName;
  ...
  std::cout << "Loaded image(" << this->_selectedFileIndex << "): " << input_impath << std::endl;
  // GT
  this->_groundTruthFrame = cv::imread(ground_truth_impath);
  std::cout << "Loaded image(" << this->_selectedFileIndex << "): " << ground_truth_impath
            << std::endl;
}
...
bool MaggicSegmentationDialog::eventFilter(QObject* f_object, QEvent* f_event) {
  ...
  if (f_event->type() == QEvent::MouseMove) {
    ...
    if (f_object == this) {
      ...
      QMouseEvent* mouseEvent = static_cast<QMouseEvent*>(f_event);
      // std::cout << "(" << qrect.x() << " " << qrect.y() << " " << qrect.width() << " " <<
      // qrect.height() << " )" << std::endl;
      cursorInsideVisualization =
          qrect.contains(static_cast<int>(mouseEvent->x()), static_cast<int>(mouseEvent->y()));
