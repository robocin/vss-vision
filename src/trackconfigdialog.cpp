...
void TrackConfigDialog::on_treeWidget_activated(const QModelIndex& index) {
  ...
  if (this->_parentIndex == DETECTION) {
    switch (this->_index) {
      case WHEREARETHOSE: this->_vision->setDetectionAlgorithm(WHEREARE_LABEL); break;
      case NONE: break;
    }
  }
...
void TrackConfigDialog::showFrame() {
  ...
  if (this->_PositionFrame.empty() || this->_PositionFrame.cols < 1 ||
      this->_PositionFrame.rows < 1) {
  ...
  QImage qimg((uchar*) this->_PositionFrame.data,
              this->_PositionFrame.cols,
              this->_PositionFrame.rows,
              this->_PositionFrame.step,
