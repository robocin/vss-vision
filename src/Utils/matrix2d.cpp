...
Matrix2d::Matrix2d(int row, int column, std::initializer_list<double> vals) :
    _row(row),
    _column(column) {
  allocSpace();
...
Matrix2d Matrix2d::solve(Matrix2d A, Matrix2d b) {
  ...
  for (int i = 0; i < A._row; ++i) {
    if (A._value[i][i] == 0.0) {
      // pivot 0 - throw error
      throw domain_error(
          "Error: the coefficient Matrix2d has 0 as a pivot. Please fix the input and try again.");
    }
