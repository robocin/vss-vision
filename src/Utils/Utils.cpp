...
bool Utils::pairCompareSecond(const std::pair<int, double>& firstElem,
...
Utils::intersectionArc(cv::Point2d line, double h, double a, double k, double b) {
...
Point Utils::convertPositionPixelToCm(const Point& position) {
  return Point(position.x * Global::convertRatio().x,
               Field::size().y - position.y * Global::convertRatio().y);
}
