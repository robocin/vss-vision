...
int CoachUtils::robotCountInDefensiveField(std::vector<Entity> &entities)
{
  ...
  for(int i=ROBOT1; i<=ROBOT3; i++) {
    if (entities[i].getPositionMatrix().x < Field::middle().x) count++;
  }
...
fieldWall CoachUtils::isNearToWall(cv::Point2d position)
{
  ...
  if (position.y >= Field::goalMin().y+ (Global::halfAxis()*4)
    ...
    if (position.x <= Field::min().x+ (Global::halfAxis()*4)/*Constants::fieldLimits.ourGoalXMin*/) res =  LEFT_WALL;
    if (position.x >= /*Constants::fieldLimits.enemyGoalXMax*/Field::goalMin().x+ (Global::halfAxis()*4)) res = RIGHT_WALL;


  } else {
    // if is near to right wall or left wall
    if (position.x <= Field::min().x + (Global::halfAxis()*4)/*Constants::fieldLimits.xMin*/) res = LEFT_WALL;
    if (position.x >= Field::max().x - (Global::halfAxis()*4)/*Constants::fieldLimits.xMax*/) res = RIGHT_WALL;
  }

  // if is near to bottom wall
  if (position.y <= Field::min().y + (Global::halfAxis()*4)/*Constants::fieldLimits.yMin*/ && res != RIGHT_WALL) res = BOTTOM_WALL;

  // if is near to upper wall
  if (position.y >= Field::max().y - (Global::halfAxis()*4)/*Constants::fieldLimits.yMax*/ && res != RIGHT_WALL) res = UP_WALL;
...
bool CoachUtils::canCarryBall(std::vector<Entity> &entities, int robotIndex, bool wasCarryingBall)
{
  ...
  if (!(robot_position.x > ballPosition.x)) return false;
  ...
  if(robot_rotation > M_PI/2) robot_rotation = robot_rotation - M_PI;
  else if(robot_rotation < -M_PI/2) robot_rotation = robot_rotation + M_PI;
  ...
  if((fabs(Geometry::radiansToDegrees(goalRobotAngleDifference)) < angleLimit
      && goalRobotDistance < goalRobotDistanceLimit
      && goalBallDistance < goalBallDistanceLimit)
...
/*bool CoachUtils::gkShouldLeaveArea(std::vector<Entity>& entities, Job job) {
  return (insideOurArea(entities[job.goalkeeper.index].getPositionCartesian()) &&
      (insideOurArea(entities[job.forward.index].getPositionCartesian()) || insideOurArea(entities[job.defender.index].getPositionCartesian()))&&
...
cv::Point2d CoachUtils::predictEntity(cv::Point2d position, cv::Point2d velocity, double limiter) {
  ...
  if (position.x < 0) position.x += 2*(0 - position.x);
  else if (position.x > 170) position.x += 2*(170 - position.x);
  else if (position.y > 95.0 || position.y < 45.0) {

    if (position.x < 10) position.x += 2*(10 - position.x);
    else if (position.x > 160) position.x += 2*(160 - position.x);
  }

  if (position.y < 0) position.y += 2*(0 - position.y);
  else if (position.y > 130) position.y += 2*(130 - position.y);
  else if(position.x < 10 || position.x > 160) {
    if (position.y < 45.0) position.y += 2.0*(45.0 - position.x);
    if (position.y > 95.0) position.y += 2.0*(95.0 - position.x);
  }
...
bool CoachUtils::gkShouldWaitArea(std::vector<Entity>& entities, Job job) {
  return !freeArea(entities, job) && gkCloseToArea(entities[job.goalkeeper.index].getPositionCartesian());
}
...
bool CoachUtils::gkCloseToArea(cv::Point2d pos) {
  return ((pos.x < 145) && (pos.x > 125) && // In front of area
      (pos.y > 30) && (pos.y < 100)) ||
      ((pos.x > 135) &&
      (((pos.y > 20) && (pos.y < 30)) || // One side of area
...
void CoachUtils::debugObjective(cv::Mat cameraFrame, std::vector<Entity>& entities, int job) {
  ...
  cv::circle(cameraFrame, Utils::convertPositionCmToPixel(ball_estimative), 4, cv::Scalar(255, 255, 51), 8);
  cv::circle(cameraFrame, Utils::convertPositionCmToPixel(robot_objective), 4, cv::Scalar(229, 149, 190), 8);
}
