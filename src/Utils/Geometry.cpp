...
Point Geometry::computeSegmentIntersection(const Point& t_a,
                                           const Point& t_b,
                                           const Point& t_c,
                                           const Point& t_d) {
...
Point Geometry::computeCircleCenter(Point t_a, Point t_b, Point t_c) {
  t_b = (t_a + t_b) / 2;
  t_c = (t_a + t_c) / 2;
  return computeLineIntersection(t_b,
                                 t_b + rotateCW90(t_a - t_b),
                                 t_c,
                                 t_c + rotateCW90(t_a - t_c));
}

float Geometry::circleCircleIntersectionAngle(const float& t_r1,
                                              const float& t_r2,
                                              const float& t_d) {
...
std::vector<Point> Geometry::circleCircleIntersection(const Point& t_p,
                                                      const float& t_r1,
                                                      const Point& t_q,
                                                      const float t_r2) {
...
Geometry::PT Geometry::rotateCCW90(Geometry::PT p) {
  return Geometry::PT(-p.y, p.x);
}
Geometry::PT Geometry::rotateCW90(Geometry::PT p) {
  return Geometry::PT(p.y, -p.x);
}
...
bool Geometry::segInter(Geometry::PT a, Geometry::PT b, Geometry::PT c, Geometry::PT d) {
  if (collinear(a, b, c, d)) {
    if (cmp(!(a - c)) == 0 || cmp(!(a - d)) == 0 || cmp(!(b - c)) == 0 || cmp(!(b - d)) == 0)
      return true;
...
std::vector<Geometry::PT>
Geometry::circleLine(Geometry::PT a, Geometry::PT b, Geometry::PT c, float r) {
...
std::vector<Geometry::PT> Geometry::circleCircle(Geometry::PT a, Geometry::PT b, float r, float R) {
  std::vector<Geometry::PT> ret;
  float d = !(a - b);
  if (d > r + R || d + std::min(r, R) < std::max(r, R))
    return ret;
...
bool Geometry::isPointInsideSector(Geometry::PT p1,
                                   Geometry::PT center,
  ...
  return !areClockwise(sectorStart, relPoint) && areClockwise(sectorEnd, relPoint) &&
         isWithinRadius(relPoint, sectorRadius);
}

bool Geometry::isRangeInsideSector(Geometry::PT p1,
                                   Geometry::PT center,
                                   Geometry::PT sectorStart,
                                   Geometry::PT sectorEnd,
                                   float sectorRadius,
                                   float rangeRadius) {
  return isPointInsideSector(Geometry::PT(p1.x, p1.y - rangeRadius),
                             center,
                             sectorStart,
                             sectorEnd,
                             sectorRadius) ||
         isPointInsideSector(Geometry::PT(p1.x, p1.y),
                             center,
                             sectorStart,
                             sectorEnd,
                             sectorRadius) ||
         isPointInsideSector(Geometry::PT(p1.x, p1.y + rangeRadius),
                             center,
                             sectorStart,
                             sectorEnd,
                             sectorRadius);
