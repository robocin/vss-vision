...
void Logging::init() {
  ...
  auto rGeneral =
      std::make_shared<spdlog::sinks::rotating_file_sink_mt>("../src/Log/general/rotative/rotative",
                                                             SIZE_LOG,
                                                             NFILES_LOG);
  auto rOthers =
      std::make_shared<spdlog::sinks::rotating_file_sink_mt>("../src/Log/others/rotative/rotative",
                                                             SIZE_LOG,
                                                             NFILES_LOG);
  auto rVision =
      std::make_shared<spdlog::sinks::rotating_file_sink_mt>("../src/Log/vision/rotative/rotative",
                                                             SIZE_LOG,
                                                             NFILES_LOG);
  auto rStrategy = std::make_shared<spdlog::sinks::rotating_file_sink_mt>(
      "../src/Log/strategy/rotative/rotative",
      SIZE_LOG,
      NFILES_LOG);
  ...
  auto fTemporaryLog =
      std::make_shared<spdlog::sinks::simple_file_sink_mt>("../src/Log/tempLog.log", true);
